...
export default function TambahStatusSerahDokumen({
  ...
  const [allStatus, setAllStatus] = useState<(string | null)[]>([]);
  const id = params.statusSerahDokumenId;

  useEffect(() => {
    async function fetchData() {
      const status = await getStatusSerahDokumenUniqe(id);
      const allStatus =
        status?.statusserahdokumen?.map((item) => item.status_serah) || [];
                    ...
                    <div className="flex flex-row gap-4 w-full justify-between p-1 items-center">
                      <FormControl>
                        <div className="flex gap-2">
                          {allStatus.includes(status.status) ? (
