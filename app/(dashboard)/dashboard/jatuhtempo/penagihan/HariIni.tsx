...
export default async function HariIni({
  searchParams,
}: {
  searchParams?: {
    page?: string;
  ...
  const jatuhTempo = await getJatuhTempo(currentPage, query);
  const totalPages = await getJatuhTempoPages(query);
  ...
  const jakartaTglJtArray = jatuhTempo.map((main) => {
    const penagihanTglJt = main.tandaterimatagihan?.tgl_jt ?? null;

    return utcToZonedTime(
      zonedTimeToUtc(parseISO(penagihanTglJt?.toISOString() ?? ""), "UTC"),
      "Asia/Jakarta"
    );
  });
